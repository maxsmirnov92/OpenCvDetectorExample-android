...
public class HumanDetector {
	...
	public static ObjectDetectFrameInfo findHuman(Mat imgScene, Scalar rectColor, Scalar fontColor) {
		...
		List<ru.maxsmr.opencv.commondetector.model.graphic.Rect> locations2 = null;

		if (!locations.empty()) {

			locations2 = new ArrayList<ru.maxsmr.opencv.commondetector.model.graphic.Rect>();
			...
			for (Rect rect : locations.toList()) {
				...
				locations2.add(new ru.maxsmr.opencv.commondetector.model.graphic.Rect(rect.x, rect.y, rect.width, rect.height));
			}
		}
